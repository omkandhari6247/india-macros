...
# --------------------------------------------------------------------------- #
#  Data-status banner — be honest about synthetic vs live before any number
# --------------------------------------------------------------------------- #
_status_level, _status_msg = ie.data_status_note(
    bool(config.FRED_API_KEY), bool(config.NEWS_API_KEY))
if _status_level == "demo":
    st.error(_status_msg)
elif _status_level == "partial":
...
with tabs[0]:
    ...
    st.divider()
    st.markdown("#### 🧭 What the dashboard is saying (CFA cycle read)")
    st.markdown(ie.business_cycle_narrative(D["cycle"]))
    st.info("**Equity-market implication** — " +
            ie.equity_context(D["portfolio"]["regime"], D["cycle"],
                              D["risk"]["global_risk_level"], D["infl_in"]))
    with st.expander("How every score on this page is built"):
        for k in ("business_cycle", "consumer_demand", "rbi", "fed",
                  "global_risk", "portfolio"):
            st.markdown(ie.SCORING_NOTES[k])
...
with tabs[1]:
    ...
    st.caption(f"🧭 CFA classification logic: {D['cycle']['cfa_reasoning']}")
    st.markdown(ie.business_cycle_narrative(D["cycle"]))
    cc = st.columns([1, 1.3])
    with cc[0]:
    ...
    with st.expander("📚 How to read these (CFA framework)"):
        st.markdown("**Yield curve** — " + ie.INDICATOR_NOTES["yield_curve"])
        st.markdown("**Credit spreads** — " + ie.INDICATOR_NOTES["credit_spreads"])
        st.markdown("**PMI** — " + ie.INDICATOR_NOTES["pmi"])
        st.markdown("**Credit growth** — " + ie.INDICATOR_NOTES["credit_growth"])
...
with tabs[2]:
    ...
    with cc2[1]:
        ...
        st.plotly_chart(line(["india_diesel", "india_petrol"], "Fuel Consumption"),
                        use_container_width=True)
    with st.expander("📚 How the demand score works & what each input means"):
        st.markdown(ie.SCORING_NOTES["consumer_demand"])
        for k in ("gst", "upi", "auto", "petroleum"):
            st.markdown(f"**{k.upper()}** — {ie.INDICATOR_NOTES[k]}")
...
with tabs[6]:
    ...
    st.info("**Equity read** — " + ie.equity_context(
        p["regime"], D["cycle"], D["risk"]["global_risk_level"], D["infl_in"]))
    ...
    with st.expander("📚 Regime methodology & full phase playbook"):
        st.markdown(ie.SCORING_NOTES["portfolio"])
        pb = ie.PHASE_PLAYBOOK.get(D["cycle"]["phase"], {})
        st.markdown(f"**Current phase — {D['cycle']['phase']}**")
        st.markdown(f"- *Backdrop:* {pb.get('desc','')}")
        st.markdown(f"- *Equities:* {pb.get('equities','')}")
        st.markdown(f"- *Bonds:* {pb.get('bonds','')}")
        st.markdown(f"- *Tilt:* {pb.get('tilt','')}")
